...
const Jacket = () => {
  ...
  const [filterHoddie, setFilterHoddie] = useState("Jacket");
  ...
  useEffect(() => {
    ...
    const onlyJacket = useProduct.filter(
      (product) =>
        product.category.toLowerCase().trim() ===
        filterHoddie.toLowerCase().trim()
    );

    console.log("Filtered Products:", onlyJacket);

    setJacket(onlyJacket);
    setIsLoading(false);
  ...
  return (
    <div className="md:max-w-[1100px] mx-auto pt-8">
      <>
        <ProductInfo
          title={"Jacket"}
          paragraph={`Introducing Diagram's vibrant Jacket Collection – where premium
cotton meets bold style and comfort!

Crafted from fine cotton fabric, our Jackets offer unrivaled softness and breathability,
ensuring you stay cozy and stylish all day long. Each Jacket boasts a unique blend of
